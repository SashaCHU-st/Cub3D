...
/*   Updated: 2024/10/18 17:31:14 by aheinane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (wall->side == 'h')
	{
		if (wall->ray_dir.x == 0)
			wall->distance = (wall->map.x - data->texture.play.x
					+ (1 - cur.hori.step) / 2) / EPSILON;
		else
			wall->distance = (wall->map.x - data->texture.play.x
					+ (1 - cur.hori.step) / 2) / wall->ray_dir.x;
	...
	else
	{
		if (wall->ray_dir.y == 0)
			wall->distance = (wall->map.y - data->texture.play.y
					+ (1 - cur.vert.step) / 2) / EPSILON;
		else
			wall->distance = (wall->map.y - data->texture.play.y
					+ (1 - cur.vert.step) / 2) / wall->ray_dir.y;
